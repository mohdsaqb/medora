        ...
        alertify.confirm(
            ...
            ok => {
                PatientService.deletePatient(patientid).then(() => {
                    ...
                    // Report success only once the server has confirmed it.
                    alertify.success('Patient deleted');
                }).catch(error => {
                    const status = error.response && error.response.status;
                    if (status === 401) alertify.error('Sign in to delete a patient');
                    else if (status === 403) alertify.error('Only an admin can delete a patient');
                    else alertify.error('Could not delete the patient');
                });
            },
                                        ...
                                        <td>
                                            <div className="btn-group" role="group">
                                                ...
                                                <div className="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                                    ...
                                                    <button
                                                        className="dropdown-item"
                                                        onClick={() => this.editPatient(patient.patientid)}
                                                        disabled={!AuthService.canEditPatients()}
                                                        title={AuthService.canEditPatients() ? '' : 'Sign in to edit'} > Edit</button>
                                                    <div className="dropdown-divider"></div>
                                                    <button
                                                        className="dropdown-item"
                                                        onClick={() => this.deletePatient(patient.patientid)}
                                                        disabled={!AuthService.canDelete()}
                                                        title={AuthService.canDelete() ? '' : 'Admins only'}> Delete </button>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
