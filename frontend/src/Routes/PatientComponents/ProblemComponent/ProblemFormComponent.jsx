...
class ProblemFormComponent extends Component {
    ...
    loadStatus() {
        ProblemService.getProblemStatus().then(res => {
            // Build the option objects here and keep them in state. A
            // module-level array filled after setState never triggers a
            // re-render, so the dropdown stayed empty.
            this.setState({
                problemStatuses: res.data,
                statusOptions: res.data.map(status => ({ value: status, label: status }))
            });
        });
    }
